...
function bindEvents(){
  ...
  document.querySelectorAll('.subtab-btn').forEach(btn=>btn.addEventListener('click',()=>{
    ...
    document.querySelectorAll('.subtab-btn').forEach(x=>x.classList.toggle('active',x===btn));
    els.blockSelectWrap.style.display=appState.mode==='from-block'?'':'none';
    if(appState.mode==='from-block') updateCalcInputsFromBlock();
    calculate();
  }));
  ...
  els.treatmentPlaceFilter.addEventListener('change',applyTreatmentsFilters);
...
function openPanel(name){
  closeTabs();
  document.querySelectorAll('.panel-content').forEach(sec=>sec.classList.remove('active'));
...
function calculate(){
  ...
  els.pressureLower.textContent=pressure.lower?`Нижняя точка: ${fmt(pressure.lower.flow,3)} л/мин при ${fmt(pressure.lower.pressure,0)} бар`:'';
  els.pressureUpper.textContent=pressure.upper?`Верхняя точка: ${fmt(pressure.upper.flow,3)} л/мин при ${fmt(pressure.upper.pressure,0)} бар`:'';
}
function sortMeasurements(list){
  return [...list].map(x=>({pressure:toNum(x.pressure),flow:toNum(x.flow)})).filter(x=>x.pressure>0&&x.flow>0).sort((a,b)=>a.flow-b.flow);
}
function interpolatePressure(targetFlow,list){
  const rows=sortMeasurements(list);
  if(!rows.length||!isFinite(targetFlow)||targetFlow<=0) return {status:'Нет данных',pressure:null};
  if(rows.length===1) return {status:'Мало точек',pressure:rows[0].pressure,lower:rows[0],upper:rows[0]};
  if(targetFlow<rows[0].flow) return {status:'Ниже таблицы',pressure:null,lower:rows[0],upper:rows[1]};
  if(targetFlow>rows[rows.length-1].flow) return {status:'Выше таблицы',pressure:null,lower:rows[rows.length-2],upper:rows[rows.length-1]};
...
function saveMeasurement(){
  const pressure=toNum(els.measurementPressure.value), flow=toNum(els.measurementFlow.value);
...
function renderMeasurements(){
  ...
  els.measurementsBody.querySelectorAll('[data-remove]').forEach(btn=>btn.addEventListener('click',()=>{
    const idx=Number(btn.dataset.remove);
    const sorted=sortMeasurements(appState.measurements);
    const target=sorted[idx];
    appState.measurements=appState.measurements.filter(x=>!(toNum(x.pressure)===target.pressure&&toNum(x.flow)===target.flow));
    save(LS_MEASUREMENTS,appState.measurements);
    renderMeasurements();
    calculate();
  }));
...
function saveBlock(){
  const block={id:appState.editingBlockId||id(),name:els.blockName.value.trim(),culture:els.blockCulture.value.trim(),year:toNum(els.blockYear.value),rows:toNum(els.blockRows.value),scheme:els.blockScheme.value.trim(),count:toNum(els.blockCount.value),area:toNum(els.blockArea.value)};
  if(!block.name) return;
  if(appState.editingBlockId){appState.blocks=appState.blocks.map(b=>b.id===appState.editingBlockId?block:b);} else {appState.blocks.push(block);}
...
function renderBlocks(){
  ...
  appState.blocks.forEach(block=>{
    const tr=document.createElement('tr');
    tr.innerHTML=`<td>${escapeHtml(block.name)}</td><td>${escapeHtml(block.culture||'')}</td><td>${fmt(block.year,0)}</td><td>${fmt(block.rows,0)}</td><td>${escapeHtml(block.scheme||'')}</td><td>${fmt(block.count,0)}</td><td>${fmt(block.area,2)}</td><td><button class="action-btn" data-edit="${block.id}">Изменить</button><button class="action-btn" data-delete="${block.id}">Удалить</button></td>`;
    els.blocksBody.appendChild(tr);
  });
  els.blocksBody.querySelectorAll('[data-edit]').forEach(btn=>btn.addEventListener('click',()=>editBlock(btn.dataset.edit)));
  els.blocksBody.querySelectorAll('[data-delete]').forEach(btn=>btn.addEventListener('click',()=>deleteBlock(btn.dataset.delete)));
}
...
async function loadWeather(){
  ...
  let lat=toNum(els.weatherLat.value), lon=toNum(els.weatherLon.value);
...
function renderWeather(data){
  const d=data.daily;
  els.weatherCards.innerHTML='';
  if(!d||!d.time){els.weatherStatus.textContent='Нет данных погоды';return;}
  d.time.forEach((date,i)=>{
    const card=document.createElement('div');
    card.className='weather-card';
    card.innerHTML=`<div class="day">${formatDateRu(date)}</div><div>Темп.: <strong>${fmt(d.temperature_2m_max[i],1)} / ${fmt(d.temperature_2m_min[i],1)} °C</strong></div><div>Осадки: <strong>${fmt(d.precipitation_sum[i],1)} мм</strong></div><div>Вероятность дождя: <strong>${fmt(d.precipitation_probability_max[i],0)}%</strong></div><div>Ветер: <strong>${fmt(d.windspeed_10m_max[i],1)} км/ч</strong></div>`;
...
function applyStockFilters(){
  const typeFilter=els.stockTypeFilter.value||'all', sortBy=els.stockSortBy.value||'name';
  let rows=appState.stockRaw.filter(x=>x.has>0||x.bought>0||x.used>0);
  ...
  appState.stockFiltered=rows;
  renderStock();
  els.stockStatus.textContent=`Позиций: ${rows.length}`;
}
function renderStock(){
  els.stockBody.innerHTML='';
  if(!appState.stockFiltered.length){const tr=document.createElement('tr');tr.innerHTML='<td colspan="6" class="muted">Нет позиций по выбранному фильтру.</td>';els.stockBody.appendChild(tr);return;}
  appState.stockFiltered.forEach(item=>{
    const tr=document.createElement('tr');
    tr.innerHTML=`<td>${escapeHtml(item.name)}</td><td>${escapeHtml(item.type||'—')}</td><td>${escapeHtml(item.unit||'—')}</td><td>${fmt(item.bought,2)}</td><td>${fmt(item.used,2)}</td><td><strong>${fmt(item.has,2)}</strong></td>`;
...
async function loadTreatments(){
  ...
  try{
    ...
    appState.treatmentGroups=groupTreatments(appState.treatmentsRaw);
    renderTreatmentsFilters();
    applyTreatmentsFilters();
  }catch(e){els.treatmentsStatus.textContent=`Ошибка загрузки обработок: ${e.message}`;}
}
...
function groupTreatments(rows){
  ...
  for (let i = list.length - 1; i >= 0; i--) {
    ...
    const starter = Boolean(
      (row.worker && String(row.worker).trim()) ||
      (row.tankLiters !== '' && row.tankLiters !== null && typeof row.tankLiters !== 'undefined') ||
    ...
    if (starter) {
      ...
      current = {
        ...
        place: row.place || '',
        method: row.method || '',
        tankLiters: row.tankLiters || '',
        tankUnit: row.tankUnit || '',
        chemicals: [],
        notes: []
      };
    }
    ...
    const hasChemical = Boolean(
      (row.chemical && String(row.chemical).trim()) ||
      (row.chemType && String(row.chemType).trim()) ||
      (row.amount !== '' && row.amount !== null && typeof row.amount !== 'undefined')
    );

    if (hasChemical) {
      current.chemicals.push({
    ...
    if (row.note && String(row.note).trim()) {
      current.notes.push(String(row.note).trim());
    }
  }
...
function applyTreatmentsFilters(){
  ...
  appState.treatmentsFilteredGroups=groups;
  renderLastTreatment();
...
function renderLastTreatment(){
  ...
  const chemText = t.chemicals && t.chemicals.length
    ? t.chemicals.map(c => {
        const left = [c.chemical, c.chemType ? `(${c.chemType})` : ''].filter(Boolean).join(' ');
        const right = [c.amount, c.amountUnit].filter(Boolean).join(' ');
        return right ? `${left} — ${right}` : left;
      }).join('; ')
    : 'Химия не указана';

  els.lastTreatment.innerHTML =
    `<strong>${escapeHtml(t.date)}</strong> · ` +
    `${escapeHtml(t.place || 'Место не указано')} · ` +
    `${escapeHtml(t.worker || 'без имени')} · ` +
    `${escapeHtml(t.method || 'метод не указан')} · ` +
    `вода: ${escapeHtml(String(t.tankLiters || '—'))} ${escapeHtml(String(t.tankUnit || ''))} · ` +
    `${escapeHtml(chemText)}`;
}
  ...
  Object.keys(byDate).sort((a,b)=>String(b).localeCompare(String(a))).forEach(date=>{
    ...
    byDate[date].forEach(group=>{
      ...
      const chems=group.chemicals.map(c=>{
        const left=[c.chemical,c.chemType ? `(${c.chemType})` : ''].filter(Boolean).join(' ');
        const right=[c.amount,c.amountUnit].filter(Boolean).join(' ');
        return `<li>${escapeHtml(left)}${right ? ' — ' + escapeHtml(right) : ''}</li>`;
      }).join('');

      card.innerHTML=`<div class="treatment-top"><div class="treatment-title">${escapeHtml(group.place || 'Место не указано')}</div><div class="muted">${escapeHtml(group.method || 'Метод не указан')}</div></div><div class="meta-grid"><div><strong>Тракторист:</strong> ${escapeHtml(group.worker || '—')}</div><div><strong>Вода:</strong> ${escapeHtml(String(group.tankLiters || '—'))} ${escapeHtml(String(group.tankUnit || ''))}</div></div><ul class="chem-list">${chems || '<li>Химия не указана</li>'}</ul>${group.notes.length ? `<div class="muted"><strong>Примечание:</strong> ${escapeHtml(group.notes.join('; '))}</div>` : ''}`;

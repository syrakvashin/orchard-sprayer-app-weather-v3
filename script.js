...
async function loadTreatments(){
  ...
  try{
    const resp=await fetch(TREATMENTS_API_URL,{cache:'no-store'});
    const raw=await resp.json();
    appState.treatmentGroups=normalizeTreatmentsPayload(raw);
    renderTreatmentsFilters();
    applyTreatmentsFilters();
  }catch(e){
    console.error(e);
    appState.treatmentGroups=[];
    appState.treatmentsFilteredGroups=[];
...
function normalizeTreatmentsPayload(payload){
  ...
  if(Array.isArray(rows[0]&&rows[0].chemicals)){
    return sortTreatmentGroups(rows.map((group,index)=>sanitizeTreatmentGroup(group,index)).filter(Boolean));
  }
  return sortTreatmentGroups(buildTreatmentGroupsFromRows(rows));
}
function buildTreatmentGroupsFromRows(rows){
  const groups=[];
  let current=null;
  const flush=()=>{
    if(!current)return;
    current=finalizeTreatmentGroup(current);
    if(current)groups.push(current);
    current=null;
  };
  rows.forEach((rawRow,index)=>{
    const row=normalizeTreatmentRow(rawRow,index);
    if(isTreatmentSeparator(row)){
      flush();
      return;
    }
    if(!current)current=createEmptyTreatmentGroup(index);
    applyTreatmentMeta(current,row);
    if(hasChemicalData(row)){
...
function normalizeTreatmentRow(rawRow,index){
  const marker=String(rawRow.marker??rawRow.separator??rawRow.split??'').trim();
  const treatmentNumber=String(rawRow.treatmentNumber??rawRow.number??rawRow.a??'').trim();
  return {
    __index:index,
    marker,
    treatmentNumber:treatmentNumber,
    date:String(rawRow.date??rawRow.b??'').trim(),
    worker:String(rawRow.worker??rawRow.who??rawRow.tractorist??rawRow.operator??rawRow.c??'').trim(),
    place:String(rawRow.place??rawRow.plot??rawRow.section??rawRow.d??'').trim(),
    method:String(rawRow.method??rawRow.methodName??rawRow.e??'').trim(),
    tankLiters:rawRow.tankLiters??rawRow.water??rawRow.f??'',
    tankUnit:String(rawRow.tankUnit??rawRow.waterUnit??rawRow.g??'').trim(),
    chemType:String(rawRow.chemType??rawRow.kind??rawRow.h??'').trim(),
    chemical:String(rawRow.chemical??rawRow.name??rawRow.i??'').trim(),
    amount:rawRow.amount??rawRow.qty??rawRow.j??'',
    amountUnit:String(rawRow.amountUnit??rawRow.unit??rawRow.k??'').trim()
  };
}
function isTreatmentSeparator(row){
  if(row.marker==='#split'||row.treatmentNumber==='#split')return true;
  return !row.date&&!row.worker&&!row.place&&!row.method&&!String(row.tankLiters??'').trim()&&!row.tankUnit&&!row.chemType&&!row.chemical&&!String(row.amount??'').trim()&&!row.amountUnit;
}
function hasChemicalData(row){
  return !!(row.chemType||row.chemical||String(row.amount??'').trim()||row.amountUnit);
}
function createEmptyTreatmentGroup(index){
  return {__index:index,treatmentNumber:'',date:'',worker:'',place:'',method:'',tankLiters:'',tankUnit:'',chemicals:[]};
}
function applyTreatmentMeta(group,row){
  if(!group.treatmentNumber&&row.treatmentNumber&&row.treatmentNumber!=='#split')group.treatmentNumber=row.treatmentNumber;
  if(!group.date&&row.date)group.date=row.date;
  if(!group.worker&&row.worker)group.worker=row.worker;
  if(!group.place&&row.place)group.place=row.place;
  if(!group.method&&row.method)group.method=row.method;
  if((group.tankLiters===''||group.tankLiters===null||typeof group.tankLiters==='undefined')&&String(row.tankLiters??'').trim()!=='')group.tankLiters=row.tankLiters;
  if(!group.tankUnit&&row.tankUnit)group.tankUnit=row.tankUnit;
}
function finalizeTreatmentGroup(group){
  const chemicals=mergeTreatmentChemicals(group.chemicals||[]);
  if(!chemicals.length&&!group.date&&!group.worker&&!group.place&&!group.method&&!String(group.tankLiters??'').trim())return null;
  return {
    ...
    place:group.place||'',
    method:group.method||'',
    tankLiters:formatAmount(group.tankLiters),
    tankUnit:group.tankUnit||'',
    chemicals:chemicals,
    __index:group.__index||0
  };
}
function sanitizeTreatmentGroup(group,index){
  if(!group)return null;
  return finalizeTreatmentGroup({
    __index:index,
    treatmentNumber:group.treatmentNumber||group.number||'',
    date:group.date||'',
    worker:group.worker||'',
    place:group.place||'',
    method:group.method||'',
    tankLiters:group.tankLiters??group.water??'',
    tankUnit:group.tankUnit||group.waterUnit||'',
    chemicals:Array.isArray(group.chemicals)?group.chemicals:[]
  });
}
function mergeTreatmentChemicals(items){
  const map=new Map();
  (items||[]).forEach(item=>{
    const chemType=String(item.chemType||'').trim();
    const chemical=String(item.chemical||'').trim();
    const amountUnit=String(item.amountUnit||'').trim();
    const amount=parseMaybeNumber(item.amount);
    if(!chemType&&!chemical&&amount===null&&!amountUnit)return;
    ...
    }else if((current.amount===null||current.amount==='')&&amount!==null){
...
function sortTreatmentGroups(groups){
  return [...groups].sort((a,b)=>{
    ...
    const numCmp=parseSortableNumber(b.treatmentNumber)-parseSortableNumber(a.treatmentNumber);
...
function parseMaybeNumber(value){
  ...
  return Number.isFinite(num)?roundSmart(num):str;
}
...
function formatAmount(value){
  ...
  if(typeof parsed==='number')return formatAmount(parsed);
...
function renderLastTreatment(){
  ...
  const water=[formatAmount(t.tankLiters||''),t.tankUnit||''].filter(Boolean).join(' ');
  els.lastTreatment.innerHTML=`<strong>${escapeHtml(t.date||'')}</strong> · ${escapeHtml(t.place||'Место не указано')} · ${escapeHtml(t.worker||'без имени')} · ${escapeHtml(t.method||'метод не указан')} · вода: ${escapeHtml(water||'—')}`;
}
  ...
  Object.keys(byDate).sort((a,b)=>String(b).localeCompare(String(a))).forEach(date=>{
    ...
    byDate[date].forEach(group=>{
      ...
      const chems=(group.chemicals||[]).map(c=>{
        ...
        const right=[formatAmount(c.amount),c.amountUnit].filter(Boolean).join(' ');
        return `<li>${escapeHtml(left)}${right?' — '+escapeHtml(right):''}</li>`;
      }).join('');
      const water=[formatAmount(group.tankLiters||''),group.tankUnit||''].filter(Boolean).join(' ');
      card.innerHTML=`<div class="treatment-top"><div class="treatment-title">${escapeHtml(group.place||'Место не указано')}</div><div class="muted">${escapeHtml(group.method||'Метод не указан')}</div></div><div class="meta-grid"><div><strong>Тракторист:</strong> ${escapeHtml(group.worker||'—')}</div><div><strong>Вода:</strong> ${escapeHtml(water||'—')}</div></div><ul class="chem-list">${chems||'<li>Химия не указана</li>'}</ul>`;

...
let appState={mode:'from-block',activeTab:'calc',measurements:load(LS_MEASUREMENTS,defaultMeasurements),blocks:load(LS_BLOCKS,defaultBlocks),editingBlockId:null,stockRaw:[],stockFiltered:[]};
const els={};
...
function saveMeasurement(){
  ...
  els.measurementPressure.value=''; els.measurementFlow.value='';
  renderMeasurements(); calculate();
}
...
function saveBlock(){
  ...
  appState.editingBlockId=null; els.blockEditNotice.textContent=''; clearBlockForm(); save(LS_BLOCKS,appState.blocks); renderBlocks(); renderBlockSelect(); calculate();
}
...
function editBlock(blockId){
  ...
  els.blockName.value=block.name||''; els.blockCulture.value=block.culture||''; els.blockYear.value=block.year||''; els.blockRows.value=block.rows||''; els.blockScheme.value=block.scheme||''; els.blockCount.value=block.count||''; els.blockArea.value=block.area||'';
...
async function loadWeather(){
  ...
  els.weatherStatus.textContent='Загрузка погоды...'; els.weatherCards.innerHTML='';
  try{
    if(city){
      const geoUrl=`https://geocoding-api.open-meteo.com/v1/search?name=${encodeURIComponent(city)}&count=1&language=ru&format=json`;
      const geoResp=await fetch(geoUrl); const geoData=await geoResp.json();
      ...
      els.weatherLat.value=lat; els.weatherLon.value=lon;
    ...
    renderWeather(data); els.weatherStatus.textContent='';
  }catch(e){els.weatherStatus.textContent=`Ошибка погоды: ${e.message}`;}
}
function renderWeather(data){
  const d=data.daily; els.weatherCards.innerHTML='';
...
async function loadAirMQ(){
  ...
  try{
    ...
    const payload={
      ...
      variables:{id:AIRMQ_LOCATION_ID,tFrom:from.toISOString(),tTo:now.toISOString(),intervalM:10}
    };
...
async function loadStock(){
  ...
  try{
    const resp=await fetch(STOCK_API_URL); const raw=await resp.json();
    const rows=Array.isArray(raw)?raw:(raw.data||[]);
    appState.stockRaw=rows.map(normalizeStockItem).filter(x=>x.name);
    renderStockTypeOptions(); applyStockFilters();
  }catch(e){els.stockStatus.textContent=`Ошибка загрузки склада: ${e.message}`;}
}
function normalizeStockItem(item){
  const keys=Object.keys(item||{});
  const get=(...variants)=>{for(const v of variants){if(item[v]!==undefined)return item[v];const found=keys.find(k=>normalizeKey(k)===normalizeKey(v));if(found)return item[found];}return '';};
...
function renderStockTypeOptions(){
  const types=[...new Set(appState.stockRaw.map(x=>x.type).filter(Boolean))].sort((a,b)=>a.localeCompare(b,'ru'));
  els.stockTypeFilter.innerHTML='<option value="all">Все виды</option>';
  types.forEach(type=>{const opt=document.createElement('option'); opt.value=type; opt.textContent=type; els.stockTypeFilter.appendChild(opt);});
}
function applyStockFilters(){
  const typeFilter=els.stockTypeFilter.value||'all';
  const sortBy=els.stockSortBy.value||'name';
  let rows=appState.stockRaw.filter(x=>x.has>0||x.bought>0||x.used>0);
  if(typeFilter!=='all') rows=rows.filter(x=>x.type===typeFilter);
  rows.sort((a,b)=>{
    if(sortBy==='type') return cmp(a.type,b.type)||cmp(a.name,b.name);
    if(sortBy==='has_desc') return b.has-a.has||cmp(a.name,b.name);
    if(sortBy==='has_asc') return a.has-b.has||cmp(a.name,b.name);
    if(sortBy==='used_desc') return b.used-a.used||cmp(a.name,b.name);

...
async function loadTreatments(){
  ...
  try{
    ...
    appState.treatmentsRaw=rows||[];
    appState.treatmentGroups=normalizeTreatmentGroups(rows);
...
function normalizeTreatmentGroups(rows){
  if(!Array.isArray(rows)) return [];
  if(rows.some(x=>Array.isArray(x&&x.chemicals))){
    return rows.map(normalizeReadyGroup).filter(isValidTreatmentGroup).sort(sortTreatmentGroupsDesc);
  }
  return groupTreatmentsFromRows(rows).sort(sortTreatmentGroupsDesc);
}
function normalizeReadyGroup(x){
  return {
    treatmentNumber: cleanText(x.treatmentNumber||x.number||''),
    date: cleanText(x.date||''),
    worker: cleanText(x.worker||x.who||''),
    place: cleanText(x.place||x.plot||''),
    method: cleanText(x.method||''),
    tankLiters: formatAmountValue(x.tankLiters||x.water||''),
    tankUnit: cleanText(x.tankUnit||x.waterUnit||''),
    chemicals: normalizeChemicals(Array.isArray(x.chemicals)?x.chemicals:[]),
    notes: Array.isArray(x.notes)?x.notes.filter(Boolean):[]
  };
}
function groupTreatmentsFromRows(rows){
  const groups=[];
  let current=createEmptyTreatmentGroup();
  rows.forEach(raw=>{
    const row=normalizeTreatmentRow(raw);
    if(isSplitRow(row)){
      pushTreatmentGroup(groups,current);
      current=createEmptyTreatmentGroup();
      return;
    }
    if(!hasMeaningfulTreatmentRow(row)) return;
    if(!current.date&&row.date) current.date=row.date;
    if(!current.treatmentNumber&&row.treatmentNumber) current.treatmentNumber=row.treatmentNumber;
    if(!current.worker&&row.worker) current.worker=row.worker;
    if(!current.place&&row.place) current.place=row.place;
    if(!current.method&&row.method) current.method=row.method;
    if(!current.tankLiters&&row.tankLiters) current.tankLiters=row.tankLiters;
    if(!current.tankUnit&&row.tankUnit) current.tankUnit=row.tankUnit;
    if(row.chemical||row.chemType||row.amount){
...
function createEmptyTreatmentGroup(){
  return {treatmentNumber:'',date:'',worker:'',place:'',method:'',tankLiters:'',tankUnit:'',chemicals:[],notes:[]};
}
function pushTreatmentGroup(groups,group){
  const normalized={
    treatmentNumber: cleanText(group.treatmentNumber||''),
    date: cleanText(group.date||''),
    worker: cleanText(group.worker||''),
    place: cleanText(group.place||''),
    method: cleanText(group.method||''),
    tankLiters: formatAmountValue(group.tankLiters||''),
    tankUnit: cleanText(group.tankUnit||''),
    chemicals: normalizeChemicals(group.chemicals||[]),
    notes: Array.isArray(group.notes)?group.notes.filter(Boolean):[]
  };
  if(isValidTreatmentGroup(normalized)) groups.push(normalized);
}
function normalizeTreatmentRow(x){
  return {
    treatmentNumber: cleanText(x.treatmentNumber||x.number||x['номер обработки']||x.a||''),
    date: cleanText(x.date||x['дата']||x.b||''),
    worker: cleanText(x.worker||x['кто проводил']||x['тракторист']||x.c||''),
    place: cleanText(x.place||x['участок']||x.d||''),
    method: cleanText(x.method||x['метод внесения']||x.e||''),
    tankLiters: formatAmountValue(x.tankLiters||x.water||x['вода']||x.f||''),
    tankUnit: cleanText(x.tankUnit||x.waterUnit||x['ед. изм. воды']||x.g||''),
    chemType: cleanText(x.chemType||x['вид химии']||x.h||''),
    chemical: cleanText(x.chemical||x['химия']||x.i||''),
    amount: formatAmountValue(x.amount||x['количество']||x.j||''),
    amountUnit: cleanText(x.amountUnit||x['ед. изм.']||x.k||''),
    marker: cleanText(x.marker||x.split||x.separator||'')
  };
}
function isSplitRow(row){
  const marker=(row.marker||row.treatmentNumber||'').toString().trim().toLowerCase();
  return marker==='#split'||marker==='---'||marker==='===';
}
function hasMeaningfulTreatmentRow(row){
  return Boolean(row.treatmentNumber||row.date||row.worker||row.place||row.method||row.tankLiters||row.tankUnit||row.chemType||row.chemical||row.amount||row.amountUnit);
}
function normalizeChemicals(list){
  const merged=new Map();
  list.forEach(item=>{
    const chemType=cleanText(item.chemType||'');
    const chemical=cleanText(item.chemical||'');
    const amountUnit=cleanText(item.amountUnit||'');
    const amount=formatAmountValue(item.amount||'');
    if(!chemical&&!chemType&&!amount) return;
    const key=[chemical.toLowerCase(),chemType.toLowerCase(),amountUnit.toLowerCase()].join('|');
    if(!merged.has(key)){
      merged.set(key,{chemType,chemical,amount,amountUnit});
      return;
    }
    const current=merged.get(key);
    const a=toNumberSafe(current.amount);
    const b=toNumberSafe(amount);
    if(a!==null&&b!==null){
      current.amount=formatAmountValue(a+b);
...
function isValidTreatmentGroup(group){
  return Boolean(group.date||group.worker||group.place||group.method||group.tankLiters||(group.chemicals&&group.chemicals.length));
}
function sortTreatmentGroupsDesc(a,b){
  const dateCmp=String(b.date||'').localeCompare(String(a.date||''));
  if(dateCmp!==0) return dateCmp;
  return String(b.treatmentNumber||'').localeCompare(String(a.treatmentNumber||''), 'ru', {numeric:true});
}
...
function formatAmountValue(value){
  ...
  if(n===null) return cleanText(value);
...
function renderTreatmentsFilters(){const places=[...new Set(appState.treatmentGroups.map(x=>x.place).filter(Boolean))].sort((a,b)=>a.localeCompare(b,'ru'));const chems=[...new Set(appState.treatmentGroups.flatMap(x=>x.chemicals.map(c=>c.chemical)).filter(Boolean))].sort((a,b)=>a.localeCompare(b,'ru'));const types=[...new Set(appState.treatmentGroups.flatMap(x=>x.chemicals.map(c=>c.chemType)).filter(Boolean))].sort((a,b)=>a.localeCompare(b,'ru'));fillSelect(els.treatmentPlaceFilter,'Все участки',places);fillSelect(els.treatmentChemFilter,'Вся химия',chems);fillSelect(els.treatmentTypeFilter,'Все виды',types);}
function fillSelect(select,firstLabel,items){select.innerHTML='';const first=document.createElement('option');first.value='all';first.textContent=firstLabel;select.appendChild(first);items.forEach(item=>{const opt=document.createElement('option');opt.value=item;opt.textContent=item;select.appendChild(opt);});}
function applyTreatmentsFilters(){const place=els.treatmentPlaceFilter.value||'all',chem=els.treatmentChemFilter.value||'all',type=els.treatmentTypeFilter.value||'all';let groups=[...appState.treatmentGroups];if(place!=='all')groups=groups.filter(x=>x.place===place);if(chem!=='all')groups=groups.filter(x=>x.chemicals.some(c=>c.chemical===chem));if(type!=='all')groups=groups.filter(x=>x.chemicals.some(c=>c.chemType===type));groups.sort((a,b)=>String(b.date).localeCompare(String(a.date)));appState.treatmentsFilteredGroups=groups;renderLastTreatment();renderTreatmentsDays();els.treatmentsStatus.textContent=`Обработок: ${groups.length}`;}
function renderLastTreatment(){if(!appState.treatmentsFilteredGroups.length){els.lastTreatment.textContent='Нет данных';return;}const t=appState.treatmentsFilteredGroups[0];els.lastTreatment.innerHTML=`<strong>${escapeHtml(t.date)}</strong> · ${escapeHtml(t.place)} · ${escapeHtml(t.worker || 'без имени')} · ${escapeHtml(t.method || 'метод не указан')} · вода: ${escapeHtml(String(t.tankLiters || ''))} ${escapeHtml(String(t.tankUnit || ''))}`;}
function renderTreatmentsDays(){els.treatmentsDays.innerHTML='';if(!appState.treatmentsFilteredGroups.length){els.treatmentsDays.innerHTML='<div class="muted">Нет обработок по выбранному фильтру.</div>';return;}const byDate={};appState.treatmentsFilteredGroups.forEach(group=>{if(!byDate[group.date])byDate[group.date]=[];byDate[group.date].push(group);});Object.keys(byDate).sort((a,b)=>String(b).localeCompare(String(a))).forEach(date=>{const day=document.createElement('div');day.className='day-group';const title=document.createElement('div');title.className='day-title';title.textContent=date;day.appendChild(title);byDate[date].forEach(group=>{const card=document.createElement('div');card.className='treatment-card';const chems=group.chemicals.map(c=>{const left=[c.chemical,c.chemType ? `(${c.chemType})` : ''].filter(Boolean).join(' ');const right=[c.amount,c.amountUnit].filter(Boolean).join(' ');return `<li>${escapeHtml(left)}${right ? ' — ' + escapeHtml(right) : ''}</li>`;}).join('');card.innerHTML=`<div class="treatment-top"><div class="treatment-title">${escapeHtml(group.place || 'Место не указано')}</div><div class="muted">${escapeHtml(group.method || 'Метод не указан')}</div></div><div class="meta-grid"><div><strong>Тракторист:</strong> ${escapeHtml(group.worker || '—')}</div><div><strong>Вода:</strong> ${escapeHtml(String(group.tankLiters || '—'))} ${escapeHtml(String(group.tankUnit || ''))}</div></div><ul class="chem-list">${chems || '<li>Химия не указана</li>'}</ul>${group.notes.length ? `<div class="muted"><strong>Примечание:</strong> ${escapeHtml(group.notes.join('; '))}</div>` : ''}`;day.appendChild(card);});els.treatmentsDays.appendChild(day);});}

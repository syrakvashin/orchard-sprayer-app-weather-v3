...
function bindElements(){
  [
    ...
    'resultCulture','resultArea','resultLiters','resultTanks','resultTotalFlow','resultFlowPerNozzle','resultPressure','pressureStatus','pressureLower','pressureUpper',
    'blockSelectWrap','measurementPressure','measurementFlow','measurementsBody','saveMeasurementBtn',
    'blockName','blockCulture','blockYear','blockRows','blockScheme','blockCount','blockArea','saveBlockBtn','blocksBody','blockEditNotice',
    ...
    'treatmentsStatus','treatmentPlaceFilter','treatmentChemFilter','treatmentTypeFilter','lastTreatment','treatmentsDays'
...
function bindEvents(){
  ...
  ['normInput','speedInput','spacingInput','nozzlesInput','tankInput','cultureInput','areaInput']
...
function clearBlockForm(){
  ['blockName','blockCulture','blockYear','blockRows','blockScheme','blockCount','blockArea'].forEach(id=>els[id].value='');
}
...
function renderWeather(data){
  ...
  d.time.forEach((date,i)=>{
    ...
    card.innerHTML=`<div class="day">${formatDateRu(date)}</div>
...
function normalizeStockItem(item){
  ...
  return {
    ...
    type:String(get('Вид химии','Вид_химии','type')).trim(),
    unit:String(get('Ед_изм','Ед изм','Ед.изм.','unit')).trim(),
    bought:toNum(get('Куплено','bought')),
    used:toNum(get('Израсходовано','used')),
    has:toNum(get('Есть','Остаток','has'))
  };
}

function normalizeKey(value){
  return String(value).toLowerCase().replaceAll('_','').replaceAll(' ','').replaceAll('.','');
}
...
function renderStockTypeOptions(){
  ...
  fillSelect(els.stockTypeFilter,'Все виды',types);
}
...
function renderStock(){
  els.stockBody.innerHTML='';
  if(!appState.stockFiltered.length){
    const tr=document.createElement('tr');
    tr.innerHTML='<td colspan="6" class="muted">Нет позиций по выбранному фильтру.</td>';
...
async function loadTreatments(){
  ...
  try{
    ...
    appState.treatmentRows=(rows||[]).map(x=>({
      treatmentNumber: String(x.treatmentNumber ?? '').trim(),
      date: String(x.date ?? '').trim(),
      worker: String(x.worker ?? '').trim(),
      place: String(x.place ?? '').trim(),
      method: String(x.method ?? '').trim(),
      tankLiters: x.tankLiters ?? '',
      tankUnit: String(x.tankUnit ?? '').trim(),
      chemType: String(x.chemType ?? '').trim(),
      chemical: String(x.chemical ?? '').trim(),
      amount: x.amount ?? '',
      amountUnit: String(x.amountUnit ?? '').trim()
    })).filter(x=>x.treatmentNumber || x.date || x.worker || x.chemical);

    appState.treatmentGroups=buildTreatmentGroups(appState.treatmentRows);
...
function buildTreatmentGroups(rows){
  const map=new Map();

  rows.forEach(row=>{
    const key = row.treatmentNumber || `${row.date}|${row.worker}|${row.place}|${row.method}|${row.tankLiters}`;
    if(!map.has(key)){
      map.set(key,{
    ...
    if(row.chemical || row.chemType || row.amount !== '' || row.amountUnit){
  ...
  return Array.from(map.values()).sort((a,b)=>{
    ...
    return toNum(b.treatmentNumber)-toNum(a.treatmentNumber);
  });
}
...
function applyTreatmentFilters(){
  const place=els.treatmentPlaceFilter.value||'all';
  const chem=els.treatmentChemFilter.value||'all';
  const type=els.treatmentTypeFilter.value||'all';
  ...
  appState.filteredTreatmentGroups=groups;
  renderLastTreatment();
  renderTreatmentDays();
...
function renderLastTreatment(){
  if(!appState.filteredTreatmentGroups.length){
    els.lastTreatment.textContent='Нет данных';
    return;
  }

  const t=appState.filteredTreatmentGroups[0];
  const chemText=t.chemicals.length
    ? t.chemicals.map(c=>{
  ...
  els.lastTreatment.innerHTML=
    ...
    `№ ${escapeHtml(String(t.treatmentNumber || '—'))} · ` +
...
function renderTreatmentDays(){
  ...
  const byDate={};
  appState.filteredTreatmentGroups.forEach(group=>{
    ...
    byDate[date].forEach(group=>{
      ...
      card.innerHTML=
        `<div class="treatment-top">` +
          `<div class="treatment-title">№ ${escapeHtml(String(group.treatmentNumber || '—'))} · ${escapeHtml(group.place || 'Место не указано')}</div>` +
          `<div class="muted">${escapeHtml(group.method || 'Метод не указан')}</div>` +
        `</div>` +
        `<div class="meta-grid">` +
          `<div><strong>Кто проводил:</strong> ${escapeHtml(group.worker || '—')}</div>` +
          `<div><strong>Вода:</strong> ${escapeHtml(String(group.tankLiters || '—'))} ${escapeHtml(String(group.tankUnit || ''))}</div>` +
        `</div>` +
        `<ul class="chem-list">${chems || '<li>Химия не указана</li>'}</ul>`;

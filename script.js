...
async function loadTreatments(){
  ...
  try{
    ...
    const grouped=rows.map(normalizeTreatmentGroup).filter(x=>x.date||x.worker||x.place||x.chemicals.length);
    appState.treatmentGroups=grouped.sort(sortTreatmentGroupsDesc);
...
function normalizeTreatmentGroup(item){
  const chemicalsRaw=Array.isArray(item?.chemicals)?item.chemicals:[];
  const chemicals=chemicalsRaw.map(c=>({
    chemType:String(c?.chemType||'').trim(),
    chemical:String(c?.chemical||'').trim(),
    amount:cleanDisplayNumber(c?.amount),
    amountUnit:String(c?.amountUnit||'').trim()
  })).filter(c=>c.chemical||c.chemType||c.amount);
  return {
    treatmentNumber:cleanDisplayNumber(item?.treatmentNumber),
    date:String(item?.date||'').trim(),
    worker:String(item?.worker||'').trim(),
    place:String(item?.place||'').trim(),
    method:String(item?.method||'').trim(),
    tankLiters:cleanDisplayNumber(item?.tankLiters),
    tankUnit:String(item?.tankUnit||'').trim(),
    places:Array.isArray(item?.places)?item.places.map(x=>String(x||'').trim()).filter(Boolean):[],
    methods:Array.isArray(item?.methods)?item.methods.map(x=>String(x||'').trim()).filter(Boolean):[],
    notes:Array.isArray(item?.notes)?item.notes.map(x=>String(x||'').trim()).filter(Boolean):[],
    chemicals
  };
}
function cleanDisplayNumber(value){
  ...
  return String(Math.round((num+Number.EPSILON)*1000)/1000).replace(/\.0+$/,'').replace(/(\.[0-9]*?)0+$/,'$1');
}
...
function renderTreatmentsFilters(){
  const places=[...new Set(appState.treatmentGroups.flatMap(x=>[x.place,...(x.places||[])]).filter(Boolean))].sort((a,b)=>a.localeCompare(b,'ru'));
...
function applyTreatmentsFilters(){
  ...
  if(place!=='all')groups=groups.filter(x=>x.place===place||(x.places||[]).includes(place));
  ...
  groups.sort(sortTreatmentGroupsDesc);
...
function renderLastTreatment(){
  ...
  const number=t.treatmentNumber?`№${escapeHtml(String(t.treatmentNumber))} · `:'';
  els.lastTreatment.innerHTML=`${number}<strong>${escapeHtml(t.date)}</strong> · ${escapeHtml(t.place||'место не указано')} · ${escapeHtml(t.worker||'без имени')} · ${escapeHtml(t.method||'метод не указан')} · вода: ${escapeHtml(String(t.tankLiters||'—'))} ${escapeHtml(String(t.tankUnit||''))}`;
}
function renderTreatmentsDays(){
  ...
  Object.keys(byDate).sort((a,b)=>String(b).localeCompare(String(a))).forEach(date=>{
    ...
    byDate[date].forEach(group=>{
      ...
      const places=(group.places&&group.places.length?group.places:[group.place]).filter(Boolean);
      const methods=(group.methods&&group.methods.length?group.methods:[group.method]).filter(Boolean);
      ...
      const numberLine=group.treatmentNumber?`<div class="muted small">№ обработки: ${escapeHtml(String(group.treatmentNumber))}</div>`:'';
      card.innerHTML=`${numberLine}<div class="treatment-top"><div class="treatment-title">${escapeHtml(group.worker||'Без тракториста')}</div><div class="muted">${escapeHtml(group.method||methods.join(' • ')||'Метод не указан')}</div></div><div class="meta-grid"><div><strong>Участок:</strong> ${escapeHtml(group.place||places.join(' • ')||'—')}</div><div><strong>Вода:</strong> ${escapeHtml(String(group.tankLiters||'—'))} ${escapeHtml(String(group.tankUnit||''))}</div></div><ul class="chem-list">${chems||'<li>Химия не указана</li>'}</ul>${group.notes.length?`<div class="muted"><strong>Примечание:</strong> ${escapeHtml(group.notes.join('; '))}</div>`:''}`;
